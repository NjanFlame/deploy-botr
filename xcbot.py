...
# 🚀 XeloraCloud ULTRA Branding Configuration - ENHANCED
BRAND_NAME = "XeloraCloud"
BRAND_EMOJI = "🚀"  # Updated to rocket for modern feel
LOGO_TOKEN = os.getenv('LOGO_TOKEN', 'https://i.imgur.com/xSsIERx.png')  # Default fallback logo
BRAND_TAGLINE = "⚡ Advanced Cloud Computing Platform"
BRAND_VERSION = "v2.0 - ULTRA ENHANCED"

# Auto-detect storage pool
async def detect_storage_pool():
    """Automatically detect the best available storage pool"""
    try:
        ...
        if process.returncode == 0 and stdout:
            ...
            for line in stdout.decode().strip().split('\n')[1:]:  # Skip header
                if line.strip():
                    pool_name = line.split(',')[0].strip()
                    if pool_name:
                        pools.append(pool_name)
            
            if pools:
                # Prefer common pool names
                preferred_pools = ['default', 'local', 'main', 'storage']
                for preferred in preferred_pools:
                    if preferred in pools:
                        logger.info(f"Auto-detected storage pool: {preferred}")
                        return preferred
                
                # Use first available pool
                logger.info(f"Auto-detected storage pool: {pools[0]}")
                return pools[0]
        
        logger.warning("No storage pools detected, using 'default'")
        return 'default'
        
    except Exception as e:
        logger.error(f"Failed to detect storage pools: {e}")
        return 'default'

# Initialize storage pool
DEFAULT_STORAGE_POOL = None

async def initialize_storage_pool():
    """Initialize the storage pool configuration"""
    global DEFAULT_STORAGE_POOL
    if DEFAULT_STORAGE_POOL is None:
        env_pool = os.getenv('DEFAULT_STORAGE_POOL')
        if env_pool:
            DEFAULT_STORAGE_POOL = env_pool
            logger.info(f"Using configured storage pool: {DEFAULT_STORAGE_POOL}")
        else:
            DEFAULT_STORAGE_POOL = await detect_storage_pool()
            logger.info(f"Auto-detected storage pool: {DEFAULT_STORAGE_POOL}")
    return DEFAULT_STORAGE_POOL

# 🚀 ENHANCED OS Options for VPS Creation - ULTRA MODERN
OS_OPTIONS = [
    {"label": "🔥 Ubuntu 24.04 LTS (RECOMMENDED)", "value": "ubuntu:24.04"},
    {"label": "⚡ Ubuntu 22.04 LTS (STABLE)", "value": "ubuntu:22.04"},
    {"label": "🏆 Ubuntu 20.04 LTS (LEGACY)", "value": "ubuntu:20.04"},
    {"label": "💎 Debian 12 (Bookworm) - LATEST", "value": "images:debian/12"},
    {"label": "🌟 Debian 11 (Bullseye) - STABLE", "value": "images:debian/11"},
    {"label": "🔧 Debian 13 (Trixie) - TESTING", "value": "images:debian/13"},
    {"label": "🛡️ Debian 10 (Buster) - LEGACY", "value": "images:debian/10"},
    {"label": "🚀 Alpine Linux 3.19 (MINIMAL)", "value": "images:alpine/3.19"},
    {"label": "⚙️ CentOS Stream 9", "value": "images:centos/9-Stream"},
]
...
# Helper function to generate clean hostname-friendly username
def sanitize_username(username):
    """Convert Discord username to hostname-friendly format"""
    import re
    # Remove or replace problematic characters
    clean_name = re.sub(r'[^a-zA-Z0-9]', '', username.lower())
    # Ensure it starts with a letter and limit length
    if not clean_name or not clean_name[0].isalpha():
        clean_name = 'user' + clean_name
    # Limit to 12 characters for hostname compatibility
...
def create_embed(title, description="", color=0x2b2d31):
    """🎨 Create an ULTRA-modern embed with enhanced XeloraCloud branding"""
    embed = discord.Embed(
        title=truncate_text(f"{BRAND_EMOJI} {BRAND_NAME} {BRAND_VERSION} - {title}", 256),
        description=truncate_text(f"{BRAND_TAGLINE}\n\n{description}", 4096),
        color=color,
        timestamp=datetime.now()
    )
    embed.set_thumbnail(url=LOGO_TOKEN)
    embed.set_footer(text=f"🚀 {BRAND_NAME} VPS Manager • Powered by ULTRA Cloud Technology • Advanced QEMU Optimization",
                     icon_url=LOGO_TOKEN)
    return embed
...
# 🚀 ULTRA-ENHANCED LXC execution with ADVANCED QEMU optimization and smart retry
async def execute_lxc(command, timeout=900, retries=5):  # 15 MINUTES + ADVANCED QEMU RETRY!
    """Execute LXC command with QEMU-optimized retry logic"""
    last_error = None
    
    for attempt in range(retries):
        try:
            if attempt > 0:
                logger.info(f"🔄 ADVANCED Retry attempt {attempt + 1}/{retries} for: {command[:50]}...")
                # 🚀 ENHANCED Progressive delay with QEMU optimization
                delay = min(3 * attempt ** 1.5, 25)  # Exponential backoff up to 25 seconds
                logger.info(f"⏰ Waiting {delay:.1f}s for QEMU stability...")
            ...
            if proc.returncode != 0:
                ...
                # 🔧 ADVANCED retryable errors for QEMU/LXC optimization
                retryable_errors = [
                    ...
                    "qemu monitor command failed",
                    "kvm acceleration not available"
                ]
                
                if attempt < retries - 1 and any(err in error.lower() for err in retryable_errors):
                    logger.warning(f"⚠️ Retryable error on attempt {attempt + 1}: {error[:100]}")
                    last_error = Exception(error)
                    continue
                    
                raise Exception(error)
                
            logger.info(f"✅ SUCCESS on attempt {attempt + 1}: {command[:50]}...")
            return stdout.decode().strip() if stdout else True
            
        except asyncio.TimeoutError as te:
            last_error = te
            if attempt < retries - 1:
                logger.warning(f"⏰ Timeout on attempt {attempt + 1}, retrying: {command[:50]}...")
                continue
            logger.error(f"LXC command timed out after {retries} attempts: {command} - {str(te)}")
            raise
        except Exception as e:
            last_error = e
            if attempt < retries - 1:
                logger.warning(f"❌ Error on attempt {attempt + 1}, retrying: {str(e)[:100]}")
                continue
            logger.error(f"LXC Error after {retries} attempts: {command} - {str(e)}")
            raise
...
# Optimized function to apply advanced permissions in parallel
async def apply_advanced_permissions(container_name):
    try:
        # Run all permission commands in parallel for speed
        permission_tasks = [
            execute_lxc(f"lxc config set {container_name} security.nesting true"),
            execute_lxc(f"lxc config set {container_name} security.privileged true"),
            execute_lxc(f"lxc config device add {container_name} fuse unix-char path=/dev/fuse"),
            execute_lxc(f"lxc config set {container_name} linux.kernel_modules overlay,loop")
        ]
        await asyncio.gather(*permission_tasks)
...
async def on_ready():
    ...
    # Initialize storage pool detection
    await initialize_storage_pool()
    logger.info(f"Storage pool configured: {DEFAULT_STORAGE_POOL}")
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.playing, name=f"🚀 {BRAND_NAME} | .help for commands | Advanced Cloud VPS"))
...
async def handle_no_prefix_help(message):
    ...
    # 🎨 ULTRA-MODERN Help Menu Design
    embed = create_info_embed(f"🌟 {BRAND_NAME} Command Center", f"🚀 **Advanced Cloud VPS Management System**\n💫 *Experience the future of cloud computing*")
    embed.color = 0x00ff88  # Bright modern green
    
    # 🎯 Quick Actions (Most Used)
    quick_cmds = "🔥 **Essential Commands:**\n"
    quick_cmds += "• `ping` - ⚡ System status & latency\n"
    quick_cmds += "• `myvps` - 📊 Your VPS dashboard\n" 
    quick_cmds += "• `manage` - 🎮 Interactive VPS control\n"
    quick_cmds += "• `vpsinfo <name>` - 📈 Detailed analytics\n"
    quick_cmds += "• `help` - 📚 This command center"
    add_field(embed, "⚡ Quick Access (.prefix)", quick_cmds, True)
    
    # 🌐 Modern Commands  
    modern_cmds = "✨ **Modern Interface:**\n"
    modern_cmds += "• `/ping` - Slash command version\n"
    modern_cmds += "• `/myvps` - Dashboard with autocomplete\n"
    modern_cmds += "• `/manage` - Advanced GUI controls\n"
    modern_cmds += "• `/help` - Interactive help system"
    add_field(embed, "🎯 Slash Commands", modern_cmds, True)
    
    if is_admin:
        admin_cmds = "👑 **Admin Control Panel:**\n"
        admin_cmds += "• `create <ram> <cpu> <disk> @user` - 🏗️ Deploy VPS\n"
        admin_cmds += "• `serverstats` - 📊 System monitoring\n"
        admin_cmds += "• `listall` - 🗂️ Global VPS overview\n"
        admin_cmds += "• `.set-threshold` - ⚙️ Resource limits\n"
        admin_cmds += "• `.set-status` - 🎨 Bot presence"
        add_field(embed, "🛡️ Admin Powers", admin_cmds, False)
    
    # 🎮 Feature Highlights
    features = "🚀 **Advanced Features:**\n"
    features += "• 🖥️ **Multi-OS Support** - Ubuntu, Debian variants\n"
    features += "• 📊 **Real-time Monitoring** - CPU, RAM, Disk usage\n"
    features += "• 🔐 **SSH Integration** - Secure remote access\n"
    features += "• ⚡ **QEMU Optimization** - Enhanced performance\n"
    features += "• 🛡️ **Auto-Suspension** - Resource protection\n"
    features += "• 📱 **Mobile Friendly** - Discord app compatible"
    add_field(embed, "💎 Pro Features", features, False)
    
    # 💡 Pro Tips & Shortcuts
    tips = f"💡 **Pro Tips & Shortcuts:**\n"
    tips += f"• **Instant Access:** No prefix needed - just type `ping`\n"
    tips += f"• **Smart Autocomplete:** Slash commands show suggestions\n"
    tips += f"• **Quick Management:** Type `manage` for one-click controls\n"
    tips += f"• **Live Stats:** Use `vpsinfo` for real-time metrics\n"
    tips += f"• **Support:** Contact {BRAND_NAME} team for assistance"
    add_field(embed, "🎯 Power User Tips", tips, False)
    
    # Footer enhancement
    embed.set_footer(text=f"🌟 {BRAND_NAME} VPS Manager • Powered by Advanced Cloud Tech • Type any command to start!", 
                     icon_url=LOGO_TOKEN)
...
async def handle_no_prefix_myvps(message):
    ...
    for i, vps in enumerate(vps_list):
        ...
        hostname = vps['container_name']
        text.append(f"**VPS {i+1}:** 🖥️ `{hostname}`\n└ 📊 {status} • 📋 {config}")
    add_field(embed, "🌟 Your VPS Collection", "\n".join(text), False)
    add_field(embed, "🎮 Quick Actions", "Type `manage` to start/stop/reinstall your VPS", False)
    await message.reply(embed=embed)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        await ctx.send(embed=create_error_embed("⚠️ System Error", f"🔧 An unexpected error occurred. Our {BRAND_NAME} engineers have been automatically notified and will resolve this quickly!\n\n💡 **Try these steps:**\n• Wait a moment and retry\n• Use `.help` for command info\n• Contact support if issue persists"))
...
async def my_vps(ctx):
    ...
    for i, vps in enumerate(vps_list):
        ...
        text.append(f"**VPS {i+1}:** 🖥️ `{hostname}`\n└ 📊 {status} • 📋 {config}")
    add_field(embed, "🌟 Your VPS Collection", "\n".join(text), False)
    add_field(embed, "🎮 Quick Actions", "Use `.manage` to start/stop/reinstall your VPS", False)
    await ctx.send(embed=embed)
...
async def lxc_list(ctx):
    try:
        ...
        embed = create_info_embed(f"{BRAND_NAME} LXC Containers List", result)
        await ctx.send(embed=embed)
    except Exception as e:
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        # Generate clean username for hostname
        clean_username = sanitize_username(self.user.display_name or self.user.name)
        container_name = f"{clean_username}-vps{vps_count}"
        ram_mb = self.ram * 1024
        try:
            # Step 1: Initialize container
            progress_embed = create_info_embed("Creating VPS", f"🔄 **Step 1/4:** Initializing {os_version} container...")
            ...
            # Step 2: Configure resources and permissions in parallel
            progress_embed = create_info_embed("Creating VPS", f"⚡ **Step 2/4:** Configuring resources and permissions...")
            await interaction.edit_original_response(embed=progress_embed, view=self)
            config_tasks = [
                execute_lxc(f"lxc config set {container_name} limits.memory {ram_mb}MB"),
                execute_lxc(f"lxc config set {container_name} limits.cpu {self.cpu}"),
                execute_lxc(f"lxc config device set {container_name} root size={self.disk}GB"),
                apply_advanced_permissions(container_name)
            ]
            await asyncio.gather(*config_tasks)
            
            # Step 3: Start container
            progress_embed = create_info_embed("Creating VPS", f"🚀 **Step 3/4:** Starting container...")
            await interaction.edit_original_response(embed=progress_embed, view=self)
            await execute_lxc(f"lxc start {container_name}")
            
            # Step 4: Set hostname inside container
            progress_embed = create_info_embed("Creating VPS", f"🏷️ **Step 4/4:** Setting hostname...")
            await interaction.edit_original_response(embed=progress_embed, view=self)
            try:
                # Quick hostname setup without delay
                await execute_lxc(f"lxc exec {container_name} -- hostnamectl set-hostname {container_name}")
                logger.info(f"Set hostname to {container_name} inside container")
            except Exception as e:
                logger.warning(f"Failed to set hostname inside {container_name}: {e}")
            ...
            success_embed = create_success_embed(f"{BRAND_NAME} VPS Created Successfully", 
                                                   f"🎉 VPS deployed for **{self.user.display_name}** with custom hostname!")
            add_field(success_embed, "👤 Owner", self.user.mention, True)
            add_field(success_embed, "🏷️ VPS ID", f"#{vps_count}", True)
            add_field(success_embed, "🖥️ Hostname", f"`{container_name}`", True)
            add_field(success_embed, "⚡ Resources", f"**RAM:** {self.ram}GB\n**CPU:** {self.cpu} Cores\n**Storage:** {self.disk}GB", False)
            add_field(success_embed, "💿 Operating System", os_version, True)
            add_field(success_embed, "🔧 Advanced Features", "✅ Nesting\n✅ Privileged Mode\n✅ FUSE Support\n✅ Kernel Modules\n🐳 Docker Ready", False)
            add_field(success_embed, "💡 Quick Tip", f"Your hostname is now **{container_name}** - perfect for easy identification!\nRun `sudo resize2fs /` inside VPS if needed to expand filesystem.", False)
            await interaction.followup.send(embed=success_embed)
            dm_embed = create_success_embed(f"{BRAND_NAME} VPS Created!", f"🎉 Your personalized VPS is ready with hostname **{container_name}**!")
            add_field(dm_embed, "🖥️ VPS Details", f"**VPS ID:** #{vps_count}\n**Hostname:** `{container_name}`\n**Configuration:** {config_str}\n**Status:** Running\n**OS:** {os_version}\n**Created:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", False)
            add_field(dm_embed, "🎮 Management Commands", f"• Type `manage` to control your {BRAND_NAME} VPS\n• Use SSH button for terminal access\n• Your hostname: `{container_name}` makes it easy to identify!\n• Contact {BRAND_NAME} admin for upgrades or issues", False)
...
class ManageView(discord.ui.View):
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'start':
            ...
            try:
                await execute_lxc(f"lxc stop {container_name}", timeout=30)
...
async def optimize_qemu_node():
    """🔥 OP OP OP QEMU node optimization on startup! 🔥"""
    try:
        logger.info("🔥 Starting MEGA QEMU optimization for OP OP OP performance!")
        
        # QEMU-specific system optimizations 
        optimization_commands = [
            # Network optimizations for QEMU
            "echo 'net.core.rmem_max = 268435456' >> /etc/sysctl.conf",
            "echo 'net.core.wmem_max = 268435456' >> /etc/sysctl.conf",
            "echo 'net.core.netdev_max_backlog = 5000' >> /etc/sysctl.conf",
            
            # Memory optimizations for QEMU
            "echo 'vm.swappiness = 1' >> /etc/sysctl.conf", 
            "echo 'vm.vfs_cache_pressure = 10' >> /etc/sysctl.conf",
            "echo 'vm.dirty_ratio = 15' >> /etc/sysctl.conf",
            "echo 'vm.dirty_background_ratio = 5' >> /etc/sysctl.conf",
            
            # I/O optimizations for QEMU performance
            ...
            # LXD optimizations for QEMU
            "lxc config set core.https_address '[::]:8443'",
            "lxc config set storage.zfs_pool_name default 2>/dev/null || true",
            "lxc config set core.trust_password ''",
            
            # Restart LXD for optimizations to take effect
            "systemctl restart lxd"
        ]
        
        logger.info("🚀 Applying OP OP OP optimizations...")
        for cmd in optimization_commands:
            try:
                # Use subprocess for system commands, not LXC
                result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=60)
                if result.returncode == 0:
                    logger.info(f"✅ SUCCESS: {cmd[:50]}...")
                else:
                    logger.warning(f"⚠️ SKIPPED: {cmd[:50]}... ({result.stderr[:100] if result.stderr else 'No error'})")
            except Exception as e:
                logger.warning(f"⚠️ FAILED: {cmd[:50]}... - {str(e)[:100]}")
                
        logger.info("🎉 QEMU node optimization COMPLETE! Your VPS is now OP OP OP FAST! 🚀")
        
    except Exception as e:
        logger.warning(f"Node optimization failed but continuing: {e}")

async def main():
    """Main function with OP OP OP QEMU optimization!"""
    # 🔥 OP OP OP optimization on startup! 🔥
    await optimize_qemu_node()
